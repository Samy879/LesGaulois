...
public class Scenario {
	...
	public static void main(String[] args) {
		...
		int forceInitiale;
		do {
		    forceInitiale = minus.getForce();
		    asterix.frapper(minus);

		    if (minus.getForce() > 0 && minus.getForce() == forceInitiale) {
		        minus.parler("Alors Asterix on fait moins le malin face à mon équipement, Hi ! Hi !");
		        obelix.parler("Astérix, tu veux un peu d'aide ?");
		        obelix.frapper(minus);
		    }

		} while (minus.getForce() > 0 && minus.getForce() != forceInitiale);
